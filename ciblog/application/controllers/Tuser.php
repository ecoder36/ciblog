	...
	class Tuser extends CI_Controller{
		public function index(){
 		...
 		//	print_r($data['posts']);
			...
			$this->load->view('tuser/index', $data);
...
//		$this->form_validation->set_rules('password_confirm', 'Confirm Password', 'trim|required|min_length[6]|matches[password]');
			if($this->form_validation->run() === FALSE){
				$this->load->view('templates/header');
				$this->load->view('tuser/create', $data);
				$this->load->view('templates/footer');
 			} else {

 				$this->test_model->create_user();
 			//	$this->test_model->create_user($username, $email, $password);
 				$data['msg'] = 'Created';
 				$data['title'] = 'Users List';
 		    	$data['users'] = $this->test_model->get_users();
 				$this->load->view('templates/header');
		...
		//	$data['users'] = $this->test_model->get_users($id);
			if($id === false){
			$ida = $this->input->post('id');
				$data['users'] = $this->test_model->get_users($ida);
			}else{
				$data['users'] = $this->test_model->get_users($id);
			}
			...
			$data['title'] = 'Edit user';
			$this->load->view('templates/header');
			$this->load->view('tuser/edit', $data);
			$this->load->view('templates/footer');
	//	}
			}
...
public function isEmailExist($email) {
    ...
    $this->load->model('test_model');
    $is_exist = $this->test_model->isEmailExist($email);
...
public function isUserExist($username) {
    $this->load->library('form_validation');
    $this->load->model('test_model');
    $is_exist = $this->test_model->isUserExist($username);
	...
	//	$this->form_validation->set_rules('username', 'Username', 'trim|required|alpha_numeric|min_length[4]|is_unique[user.username]', array('is_unique' => 'This username already exists. Please choose another one.'));
		...
		//$this->form_validation->set_rules('id', 'Id', 'trim|required|callback_isUserExist');
...
//		$this->form_validation->set_rules('password_confirm', 'Confirm Password', 'trim|required|min_length[6]|matches[password]');
			 ...
			 if($this->form_validation->run() === FALSE ){
			 	...
			 	$this->load->view('templates/footer');
 		 		...
 		 		$this->test_model->update_user($id);
